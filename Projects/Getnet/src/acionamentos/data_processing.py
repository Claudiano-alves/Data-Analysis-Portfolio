...
def confere_tabulacao_acionamentos(df_tab_acionamentos, df_tabulacao_aciona):
    ...
    salvar_log(f"📊 Acionamentos com DESC_ACIONA: {df_resultado['DESC_ACIONA'].notna().sum():,}", arquivo_log=LOG_ACIONAMENTOS)
    salvar_log("=" * 80, arquivo_log=LOG_ACIONAMENTOS)
...
def enriquecer_acionamentos(df_acionamentos, df_mailing_hist, df_dw_calendario, 
    ...
    if 'COD_RECUP' in df_resultado.columns:
        salvar_log(f"📊 Classificando origem do acionamento...", arquivo_log=LOG_ACIONAMENTOS)
        df_resultado['ORIGEM'] = df_resultado['COD_RECUP'].apply(
            lambda x: 'ROBÔ' if x == 1 else 'HUMANO'
        )
        salvar_log(f"   ✓ Robô: {(df_resultado['ORIGEM'] == 'ROBÔ').sum():,}", arquivo_log=LOG_ACIONAMENTOS)
        salvar_log(f"   ✓ Humano: {(df_resultado['ORIGEM'] == 'HUMANO').sum():,}", arquivo_log=LOG_ACIONAMENTOS)
    else:
