...
def data_pagamentos(df_pagamentos, df_acordos, df_mailing_hist, df_dw_calendario):
    ...
    # Cruzamento com mailing_hist e remoção das colunas duplicadas
...
def gerar_acumulado_por_dia_util(df_agrupado):
    ...
    for i, data in enumerate(datas_unicas, 1):
        ...
        # Verificar se o intervalo não está vazio
    ...
    # Verificar se há resultados antes de concatenar
    if not resultados:
...
def tratar_pagamentos_(df_pagamentos, df_acordos, df_mailing_hist, df_dw_calendario):
    df_pagamentos, df_sem_fx_atraso, df_pagamento_analitico = data_pagamentos(df_pagamentos, df_acordos, df_mailing_hist, df_dw_calendario)
...
def tratar_pagamentos(df_pagamentos, df_acordos, df_mailing_hist, df_dw_calendario):
    df_pagamentos, df_sem_fx_atraso, df_pagamento_analitico = data_pagamentos(
        df_pagamentos, df_acordos, df_mailing_hist, df_dw_calendario
    )
    df_pagamentos_funil, df_esforco, df_unique = gerar_acumulado_por_dia_util(df_pagamentos)

    # Verificar se todos os DataFrames estão vazios antes de unir
    if df_pagamentos_funil.empty and df_esforco.empty and df_unique.empty:
        salvar_log("="*60)
        salvar_log("AVISO: Não há dados de pagamentos para processar.")
        salvar_log("Retornando DataFrame vazio com estrutura padrão.")
        salvar_log("="*60)
        
        # Retornar um DataFrame vazio com a estrutura esperada
        # Usar as colunas do df_pagamentos_funil que já foi criado vazio
        df_pagamentos_funil_final = df_pagamentos_funil.copy()
    else:
        # Unir os DataFrames normalmente
        df_pagamentos_funil_final = unir_dataframes(df_pagamentos_funil, df_esforco, df_unique)

    return df_pagamentos_funil_final, df_sem_fx_atraso, df_pagamento_analitico